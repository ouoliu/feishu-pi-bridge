...
import { initialState, reduce, finalizeIfRunning, markInterrupted } from './card/run-state.js';
import { renderCard } from './card/run-renderer.js';
import type { AgentEvent, AgentRun } from './agent/types.js';
...
/** 获取最新用户消息 ID */
async function initLastMsgId(): Promise<string> {
  const t = await getToken();
  const r = await fetch(
    `https://open.feishu.cn/open-apis/im/v1/messages?container_id_type=chat&container_id=${CHAT_ID}&page_size=5&sort_type=ByCreateTimeDesc`,
...
async function handleMessage(text: string, messageId: string): Promise<void> {
  console.error(`\n📩 ${text.slice(0, 80)}`);
  ...
  const scheduleUpdate = (cardJson: string) => {
  ...
  try {
    ...
    console.error(`  ❌ 处理错误:`, (err as Error).message);
    if (cardId) {
      // 更新同一张卡片为错误状态
...
function buildCardResponse(title: string, body: string, footer?: string): string {
  ...
  try {
    ...
    const d = await r.json() as {
      code: number; data?: { items?: Array<{
        message_id: string; sender: { sender_type: string };
        body?: { content?: string };
      }> }
    };
